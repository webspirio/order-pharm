...
export default function MobileMenu({
  ...
  // `lg:hidden`, so past 1024px the sheet would otherwise be stuck open with
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className="inline-flex size-9 items-center justify-center rounded-lg border border-border text-foreground/80 transition-colors hover:bg-muted hover:text-foreground lg:hidden"
      >
        ...
        createPortal(
          <div className="fixed inset-0 z-[110] lg:hidden">
            ...
            <div
              ...
              className="absolute inset-y-0 right-0 flex w-[86%] max-w-sm flex-col gap-6 rounded-l-[2rem] border-l border-border bg-frame p-6 shadow-2xl"
            >
              ...
              <nav aria-label={navLabel} className="flex flex-col gap-1">
                {links.map((link) => (
                  <a
                    ...
                    href={link.href}
                    onClick={() => setOpen(false)}
                    className="rounded-lg px-3 py-3 text-lg font-medium tracking-[-0.02em] transition-colors hover:bg-muted"
                  >
                    {link.label}
                  </a>
                ))}
